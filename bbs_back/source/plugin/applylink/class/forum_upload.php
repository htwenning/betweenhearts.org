...
class apply_forum_upload {
	...
	function apply_forum_upload() {
		...
		if (($maxpiccount>0)&&$_G['uid']!=1) {
			$piccount = C::t('#applylink#plugin_lj_attachment')->count_by_where($where);
		...
		C::t('#applylink#plugin_lj_attachment')->insert($insert);
		$this->uploadmsg(0);
	}
