...
class table_plugin_daka extends discuz_table
{
	...
	public function fetch_by_uid($uid){
		return DB :: result_first("select count(*) from " . DB :: table($this->_table) . " where uid=$uid and curdate()=FROM_UNIXTIME(timestamp,'%Y-%m-%d')");
	}
	public function fetch_by_uid_yesterday($uid,$mytime){
		return DB :: result_first("select alldays from " . DB :: table($this->_table) . " where uid=$uid and FROM_UNIXTIME(timestamp,'%Y-%m-%d')='" . $mytime . "'");
	}
}
