...
class table_plugin_daka_user extends discuz_table
{
	...
	public function fetch_all_by_allday($curnum,$perpage){
		if($perpage){
			return DB :: fetch_all("select * from %t order by allday desc limit %d,%d",array($this->_table,$curnum,$perpage));
		}else{
			return DB :: fetch_all("select * from %t order by allday desc ",array($this->_table));
		}
	} 
